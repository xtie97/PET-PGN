...
parser.add_argument('--max_dec_len', type=int, default=500, help='Max decoding length.')  
parser.add_argument('--beam_size', type=int, default=8, help='Beam search size used in decoder.')
...
for epoch in range(1, opt['num_epoch']+1):
    ...
    if opt['use_bleu']:
        ...
        dev_score = 0 
        for ii in range(len(dev_preds)):
            scores = rouge.score(' '.join(dev_preds[0]), ' '.join(dev_gold[0]))
            rouge1 = list(scores['rouge1'])[2]
            rouge2 = list(scores['rouge2'])[2]
            rougel = list(scores['rougeL'])[2]
            dev_score += rougel #rougel
        dev_score /= len(dev_preds)
    ...
    # save
    model_file = model_save_dir + '/model_epoch{}.pt'.format(epoch)
